...
- **Проекты** — если написать не дело, а идею, бот сам это заметит, разберёт её вопросами с челленджем и заведёт проект в отдельном списке MS Todo: бриф в заметке, план шагов в чеклисте, MD-файл вложением. Отключается в `/settings`: с выключенным разбором каждое сообщение уходит в задачи напрямую, без триажа через LLM
...
| `/stats` | Статистика |
| `/settings` | Настройки: режим подтверждения, разведка, разбор идей в проекты |
